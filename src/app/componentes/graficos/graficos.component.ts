...
export class GraficosComponent implements OnInit {
  ...
  //Datos grafica Curva infectados
  chartCurvaInfectados: Chart;
  contMarzo = 0;
  contAbril = 0;
  sumaAbril = 0;
  contMayo = 0;
  ...
  depa(depto: string){
    ...
    this._graficos.getData(`?$limit=${this.limiteDatos}&$select=fecha_de_notificaci_n`)
  ...
  this.sumaSeptiembre = 0;
  this.contOctubre = 0;
  this.sumaOctubre = 0;
  this.contNoviembre = 0;
  this.sumaNoviembre = 0;
  this.contDiciembre = 0;
  this.sumaDiciembre = 0;
...
res.forEach(res => {
  ...
  if (this.contAbril != 0){
    this.sumaAbril = this.contMarzo + this.contAbril;
  }
  if (this.contMayo != 0){
    this.sumaMayo = this.sumaAbril + this.contMayo;
  }
  if (this.contJunio != 0){
    this.sumaJunio = this.sumaMayo + this.contJunio;
  }
  if (this.contJulio != 0){
    this.sumaJulio = this.sumaJunio + this.contJulio;
  }
  if (this.contAgosto != 0){
    this.sumaAgosto = this.sumaJulio + this.contAgosto;
  }
  if (this.contSeptiembre != 0){
    this.sumaSeptiembre = this.sumaAgosto + this.contSeptiembre;
  }

  if (this.contOctubre != 0){
    this.sumaOctubre = this.sumaSeptiembre + this.contOctubre;
  }
  if (this.contNoviembre != 0){
    this.sumaNoviembre = this.sumaOctubre + this.contNoviembre;
  }
  if (this.contDiciembre != 0){
    this.sumaDiciembre = this.sumaNoviembre + this.contDiciembre;
  }

}
);

// Inicio grafica curva infectados

if (this.chartCurvaInfectados != null) {
  this.chartCurvaInfectados.destroy();
}

this.chartCurvaInfectados = new Chart('CurvaLinea', {
  type: depto,
  data:{
    ...
    datasets: [{
      label: 'Curva infectados',
      data: [this.contMarzo, this.sumaAbril, this.sumaMayo, this.sumaJunio, 
             this.sumaJulio, this.sumaAgosto, this.sumaSeptiembre, this.sumaOctubre,
             this.sumaNoviembre, this.sumaDiciembre],
  ...
  ngOnInit(){
    ...
    this._graficos.getData('?$limit=' + this.limiteDatos + '&$select=sexo,atenci_n').subscribe((res: Paciente[]) => {


        //configuracion global charts
...
this._graficos.getData('?$limit=' + this.limiteDatos + 
'&$select=tipo_recuperaci_n').subscribe((res: Paciente[]) => {
...
//fin graficas recuperacion

//condicion grafica de etnia

this._graficos.getData('?$limit=' + this.limiteDatos + 
'&$select=pertenencia_etnica,atenci_n').subscribe((res: Paciente[]) => {
...
this._graficos.getData('?$limit=' + this.limiteDatos + 
'&$select=fecha_de_notificaci_n,estado').subscribe((res: Paciente[]) => {
...
this._graficos.getData('?$limit=' + this.limiteDatos + 
'&$select=edad,sexo').subscribe((res: Paciente[]) => {
...
this._graficos.getData('?$limit=' + this.limiteDatos + 
'&$select=edad,estado').subscribe((res: Paciente[]) => {
